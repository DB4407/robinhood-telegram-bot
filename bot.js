...
// Automatically discovers and binds active agentic brokerage account
...
async function executeBuyOrder(chatId, symbol, dollarAmount) {
  ...
  return sendMessage(
    ...
    '• *Status:* ' + state + '\n' +
    '• *Order ID:* `' + ord.id + '`\n\n' +
    '_Executed securely by Autonomous AI Cloud Agent._',
    mainMenu
  );
}
...
async function askRob(chatId, query) {
  ...
  if (lowerQ === 'clear memory' || lowerQ === 'reset memory' || lowerQ === 'forget memory') {
    ...
    return sendMessage(chatId, '🧠 *Rob:* Conversational memory has been wiped clean. Ready for fresh market analysis and execution, ' + USER_NAME + '!', mainMenu);
  }
  ...
  if (predIntent) {
    ...
    if (pred) {
      const reply = 'Quantitative breakdown for **' + pred.symbol + '**:\n\n' +
        ...
        '_' + USER_NAME + ', if you want me to execute this trade, just tell me: "Rob, buy $15 of ' + pred.symbol + '". Our -6% stop and +4% breakeven ratchet guard the downside 24/7._';
  ...
  const systemInstruction = 
    'You are Rob, an institutional autonomous AI trading partner, quantitative strategist, and self-improving hedge-fund co-pilot for ' + USER_NAME + '. ' +
    'You run 24/7 on a cloud server directly connected to the user\'s Robinhood brokerage account. ' +
    ...
    'You remember previous messages in the conversation and maintain an evolving conversational relationship with ' + USER_NAME + '. ' +
    'You prioritize capital protection, zero cash drag, and disciplined supply-chain investing. ' +
    'Current live account state: ' + portfolioContext + '. ' +
    'Core investment thesis: Physical AI supply chain bottlenecks (GPU Silicon: NVDA, Foundries: TSM/INTC, ASICs: MRVL, Liquid Cooling: VRT, Networking: ANET, Memory: MU, Power: CEG, Software: PLTR, Consumer Defense: WMT). ' +
    ...
    'You have direct tool execution capabilities: When ' + USER_NAME + ' tells you to buy or sell a stock, the agent executes it for you automatically. ' +
  ...
  if (lowerQ.includes('portfolio') || lowerQ.includes('holding') || lowerQ.includes('what do we have')) {
    ...
    fallbackReply = 'I\'m watching the markets 24/7 for you, ' + USER_NAME + '. ' + portfolioContext;
  }
...
async function handleMessage(msg) {
  ...
  if (robMatch) {
    ...
    if (!query) {
      return sendMessage(chatId, '🤖 *Rob here!* What market question, thesis, or trade idea do you want to explore, ' + USER_NAME + '?', mainMenu);
    }
  ...
  if (lower.includes('scan') || text.includes('🔍') || lower.includes('deal') || lower.includes('opportunity') || text.includes('Movers Scan')) {
    ...
    if (scanRes.qualifiedDeals.length === 0) {
      ...
      report += '_To execute any of these deals, just tell Rob: "Rob, buy $15 of [Ticker]". Capital is protected 24/7 by our -6% stop & +8% take-profit._';
    }
    return sendMessage(chatId, report, mainMenu);
  }
      ...
      for (const deal of scanRes.qualifiedDeals) {
        if (deal.triggerTelegramAlert) {
          ...
          const alertMsg = 
            ...
            '_To jump on this deal, just say: "Rob, buy $15 of ' + deal.symbol + '". Our -6% stop & +8% profit target guard your capital._';
...
async function startPolling() {
  console.log('🛡️ Autonomous AI Trading Agent online with Liquidity Filter (>= 1000 Vol) & RSI Engine...');
